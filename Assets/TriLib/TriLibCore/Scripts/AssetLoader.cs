...
namespace TriLibCore
{
    ...
    public static class AssetLoader
    {
...
#if (UNITY_WEBGL && !TRILIB_ENABLE_WEBGL_THREADS) || (UNITY_UWP && !TRILIB_ENABLE_UWP_THREADS) || TRILIB_FORCE_SYNC
                Async = false,
#else
                Async = true,
#endif
        ...
        private static AnimationClip ParseAnimation(AssetLoaderContext assetLoaderContext, IAnimation animation)
        {
            ...
            for (var i = animationCurveBindings.Count - 1; i >= 0; i--)
            {
                ...
                for (var j = 0; j < animationCurves.Count; j++)
                {
                    ...
                    //todo: working on it for the next release (2.1.8)
        ...
        {
            if (assetLoaderContext.Stream == null && (assetLoaderContext.IsZipFile || string.IsNullOrWhiteSpace(assetLoaderContext.Filename)))
            {
        ...
        private static void FinishLoading(AssetLoaderContext assetLoaderContext)
        {
            ...
            if (assetLoaderContext.RootModel != null && !assetLoaderContext.Options.LoadPointClouds && !assetLoaderContext.Options.ReadEnabled && (assetLoaderContext.Options.DoubleSidedMaterials || assetLoaderContext.Options.AlphaMaterialMode == AlphaMaterialMode.CutoutAndTransparent))
            {
                for (var i = 0; i < assetLoaderContext.RootModel.AllGeometryGroups.Count; i++)
                {
                    var geometryGroup = assetLoaderContext.RootModel.AllGeometryGroups[i];
                    geometryGroup.Cleanup();
                }
            }
