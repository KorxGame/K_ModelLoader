...
public static class BoundsCalculator
{
    ...
    internal static Bounds CalculateBounds(Transform root, out bool containsCanvas,
        ...
        bool abortOnCanvas = false)
    {
        ...
        foreach (Transform childTransform in root.GetComponentsInChildren<Transform>(includeInactiveObjects))
        {
    ...
    private static void ExtractBoundsCorners(Transform childTransform, BoundsCalculationMethod boundsCalculationMethod)
    {
        ...
        if (boundsCalculationMethod != BoundsCalculationMethod.ColliderOnly)
        {
            ...
            {
                rendererBoundsByTransform =
                    new KeyValuePair<Transform, Bounds>(childTransform, meshFilter.sharedMesh.bounds);
            }
            else if (childTransform is RectTransform rt)
            {
                rendererBoundsByTransform = new KeyValuePair<Transform, Bounds>(childTransform,
                    new Bounds(rt.rect.center, new Vector3(rt.rect.width, rt.rect.height, 0.1f)));
            }
            else
            {
        ...
        {
            if (AddColliderBoundsCornersToTarget(colliderByTransform) &&
                boundsCalculationMethod == BoundsCalculationMethod.ColliderOverRenderer ||
        ...
        if (boundsCalculationMethod != BoundsCalculationMethod.ColliderOnly)
        {
            if (AddRendererBoundsCornersToTarget(rendererBoundsByTransform) &&
                boundsCalculationMethod == BoundsCalculationMethod.RendererOverCollider ||
                boundsCalculationMethod == BoundsCalculationMethod.RendererOnly)
            {
                return;
            }
        }
